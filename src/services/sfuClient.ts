...
export class SFUClient {
	...
	async getTransport(socketId: string) {
		return await this.transportModel.findBySocketIdWhereNotConsumer(
			socketId
		);
	}
	...
	async joinOrCreateRoom(roomName: any, socketId: string) {
		let router1;
		let peers = [];
	...
	async joinRoom(roomName: any, socketId: string) {
		let router1;
		let peers = [];
		let room = await this.roomModel.findOneByRoomName(roomName);
		if (room) {
