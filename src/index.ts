...
let transports: Array<{ socketId: string, roomName: string, transport: WebRtcTransport, consumer: Consumer }> = []     // [ { socketId1, roomName1, transport, consumer }, ... ]
let producers: { socketId: string, roomName: string, producer: Producer }[] = []      // [ { socketId1, roomName1, producer, }, ... ]
let consumers: { socketId: string, roomName: string, consumer: Consumer }[] = []       // [ { socketId1, roomName1, consumer, }, ... ]
...
const mediaCodecs: Array<RtpCodecCapability> = [
	{
		kind: 'audio',
		mimeType: 'audio/opus',
		clockRate: 48000,
...
connections.on('connection', async socket => {
	...
	const removeItems = (items: Array<any>, socketId: string, type: string) => {
		items.forEach(item => {
	...
	socket.on('disconnect', () => {
		...
		rooms[roomName] = {
			...
			peers: rooms[roomName].peers.filter((socketId: string) => socketId !== socket.id)
		}
	})
