...
import { SocketIOConnection } from "./services/socketIoConnection";
import { getRoomHandler } from "./services/room/roomFactory";
...
const socketIOConnection = SocketIOConnection.init(server);

const io = socketIOConnection.io;
...
sfuClient.createAndGetWorker().then((worker: Worker) => {
	...
	connections.on("connection", async (socket) => {
		...
		socket.on("join_room", async ({ roomName, type }) => {
			// handle user joining a session
			console.log("handling connection");

			console.log("rooom name is as following", roomName);
			const roomHandler = getRoomHandler(type)
			const res = await roomHandler.authorizeConnection(roomName, token)
